...
def windestroy():
     tkinter.messagebox.showinfo("Quit","You have chosen to exit the program\nThank you for visiting!")
     window1.destroy()        
#_________________
#video display
def call(I):
    ...
    cap = cv2.VideoCapture('D:/Reliance School/Computer Project/videos/'+I+'.mp4') 
       
    # Check if camera opened successfully 
    if (cap.isOpened()== False):  
      tkinter.messagebox.showinfo("Video error!","Error opening video  file") 
...
def receive_audio_eng():
    ...
    with sr.Microphone(sample_rate = sample_rate,  chunk_size = chunk_size) as source:
        ...
        try:          
            ...
            tkinter.messagebox.showinfo('Input Error!!!',"Google Speech Recognition could not understand audio\nPlease enter again")
            #("Google Speech Recognition could not understand audio") 
      
        except sr.RequestError as e: 
           tkinter.messagebox.showinfo("Input Error!!!","Could not request results from Google Speech Recognition service; {0}".format(e))    
...
def receive_audio_hin():
    ...
    with sr.Microphone(sample_rate = sample_rate,  chunk_size = chunk_size) as source:
        ...
        try:          
            ...
            tkinter.messagebox.showinfo('इनपुट त्रुटि !!! ' ,   " गूगल भाषण मान्यता ऑडियो नहीं समझ सका। कृपया फिर से दर्ज करें")
            #("Google Speech Recognition could not understand audio") 
      
        except sr.RequestError as e: 
           tkinter.messagebox.showinfo('इनपुट त्रुटि !!! ' ," गूगल भाषण मान्यता सेवा से परिणाम का अनुरोध नहीं किया जा सका". format(e))    
           #print("Could not request results from Google Speech Recognition service; {0}".format(e))
...
def alphabets():
    ...
    window2.iconbitmap('D:/Reliance school/Computer project/extra/Danieledesantis-Audio-Video-Outline-Play.ico')
...
def nature():
        ...
        window3.iconbitmap('D:/Reliance school/Computer project/extra/Danieledesantis-Audio-Video-Outline-Play.ico')
...
def numbers():
        ...
        window4.iconbitmap('D:/Reliance school/Computer project/extra/Danieledesantis-Audio-Video-Outline-Play.ico')
...
def music():
        ...
        window5.iconbitmap('D:/Reliance school/Computer project/extra/Danieledesantis-Audio-Video-Outline-Play.ico')
...
def sports():
        ...
        window6.iconbitmap('D:/Reliance school/Computer project/extra/Danieledesantis-Audio-Video-Outline-Play.ico')
...
def action():
        ...
        window7.iconbitmap('D:/Reliance school/Computer project/extra/Danieledesantis-Audio-Video-Outline-Play.ico')
...
def question():
        ...
        window8.iconbitmap('D:/Reliance school/Computer project/extra/Danieledesantis-Audio-Video-Outline-Play.ico')
...
def gameRun(j):
    ...
    windows50.iconbitmap('D:/Reliance school/Computer project/extra/Danieledesantis-Audio-Video-Outline-Play.ico')
    ...
    global title_lb
    global title2_lb
    title_lb = tk.Label(windows50, text = "It's Game Time", font=('Gabriola',80,'bold'),bg='#FFBC8B')
    title_lb.place(x = 15, y = 0)
    title2_lb = tk.Label(windows50, text = "Test your knowledge of Indian Sign Language", font=('Cambria',12,'italic'),bg='#FFBC8B')
    title2_lb.place(x = 135, y = 155)
    ...
    global score1_lb
    global total1_lb
    score1_lb = tk.Label(windows50, text = "Score:", font=('Cambria',25),bg='#FFBC8B')
    total1_lb = tk.Label(windows50, text = "Total Questions:", font=('Cambria',25),bg='#FFBC8B')
    score1_lb.place(x = 750, y = 75)
    total1_lb.place(x = 750, y = 125)
    global score2_lb
    global total2_lb
    score2_lb = tk.Label(windows50, text = score, font=('Cambria',25),bg='#FFBC8B')
    total2_lb = tk.Label(windows50, text = total, font=('Cambria',25),bg='#FFBC8B')
    score2_lb.place(x = 1150, y = 75)
    total2_lb.place(x = 1150, y = 125)
...
def entry_message():
    ...
    global hs
    global id1
    sql_obj3=mysql.connector.connect(host='bwqwqvenmbwqhzwutgu6-mysql.services.clever-cloud.com',user='u7bizpwgzah61bgv',password='UKX0nnX3eNDTj86DEOTT',database='bwqwqvenmbwqhzwutgu6')
    cursor=sql_obj3.cursor()
    cursor.execute(f"select user_id from user where username='{username}'")
    id1=cursor.fetchall()
    id1=id1[0][0]               
    cursor.execute(f"select highscore from level{level} where user_id={id1}")              
    hs=cursor.fetchall()               
    hs=hs[0][0]
             
    
    if hs==0:
        tkinter.messagebox.showinfo("Welcome!",f"You are playing level {level}\nLet's see how much you can score in the first go!",parent=windows50)
    else:
        tkinter.messagebox.showinfo("Welcome!",f"You are playing level {level}\nYour highscore is {hs}\nCan you beat it?",parent=windows50)
...
def enter_ans():
    ...
    global Q
    global answer_ent
    answer_lb = tk.Label(windows50, text = "Enter your Answer",font=('Cambria',18),bg='#FFBC8B')
    answer_lb.place(x = 875, y = 300)
    answer_ent = tk.Entry(windows50, width = 25,font=('Cambria',14))
    answer_ent.place(x=875, y=360)
    answer_btn = tk.Button(windows50, text = "Enter", font=('Cambria',14), command=accept_ans,bg='#F29062')
    answer_btn.place(x = 1160, y = 354)
...
def display_next():
    global windows50
    global next_btn
    next_btn = tk.Button(windows50, text = "Next Question", font=('Cambria',20), command=next_func,bg='#F29062')
    next_btn.place(x = 900, y = 500)
...
def playAgain():
    ...
    playAgain_btn.place(x = 130, y = 450)
...
def quit_func():
    ...
    if score>hs:
        cursor.execute(f"update level{level} set highscore={score} where user_id={id1}")
        sql_obj2.commit()
        tkinter.messagebox.showinfo("Quit",f"You have set a new high score of {score}!\nExiting the game...",parent=windows50)
    else:
        tkinter.messagebox.showinfo("Quit",f"Your final score is {score}!\nExiting the game...",parent=windows50)
    sql_obj2.close()
    windows50.destroy()
...
def display_quit():
    global windows50
    global quit_btn
    quit_btn = tk.Button(windows50, text = "Quit", command = quit_func, font=('Cambria',20),bg='#F29062')
    quit_btn.place(x = 1100, y = 500)
...
def english_gen():
    ...
    english_pic1=Image.open('D:/Reliance School/Computer Project/images/mic1.png')
...
def hindi_gen():
    ...
    hindi_pic1=Image.open('D:/Reliance School/Computer Project/images/mic1.png')
...
def homepage():
    ...
    info3='This program developed using Python programming language can help cater to the needs of the\n specially abled people. We can the platform for effective communication, to learn and test our      \n knowledge of sign language.                                                                                                                       '              
    frame3=tk.Frame(Homepage_tab,bg='#FFBC8B',width=1100,height=100)
    ...
    frame4=tk.Frame(Homepage_tab,bg='#FFBC8B',width=500,height=100)
    frame4.place(x=0,y=350)
    lbl_info2=tk.Label(frame4,bg='#FFBC8B',text=info2,font=('Cambria',12))
    lbl_info2.pack()
    frame5=tk.Frame(Homepage_tab,bg='#FFBC8B',width=1100,height=100)
    frame5.place(x=200,y=435)
    lbl_info3=tk.Label(frame5,bg='#FFBC8B',text=info3,font=('Cambria',12))
    lbl_info3.pack()
    frame6=tk.Frame(Homepage_tab,bg='#FFBC8B',width=1100,height=100)
    frame6.place(x=150,y=530)
    lbl_info4=tk.Label(frame6,bg='#FFBC8B',text='Check out the variuos utilities by navigating using buttons on the menu bar!',font=('Cambria bold italic',20))
    lbl_info4.pack()
    #_______________
    ...
    frame4=tk.Frame(Homepage_tab,bg='#FFBC8B',width=500,height=180)
    frame4.place(x=930,y=300)
    about_pic1=Image.open('D:/Reliance School/Computer Project/images/about.jpg')
...
def dictionary():
    ...
    frame3.place(x=30,y=220,width=1400,height=450)
...
def game():
    ...
    level1_btn = tk.Button(frame3,text='Level 1',bg='#F29062',font=('Cambria',40),command=lambda m="level 1":gameRun(m))
    level1_btn.place(x=100,y=20)
    level2_btn = tk.Button(frame3,text='Level 2',bg='#F29062',font=('Cambria',40),command=lambda m="level 2":gameRun(m))
    level2_btn.place(x=480,y=20)
    level3_btn = tk.Button(frame3,text='Level 3',bg='#F29062',font=('Cambria',40),command=lambda m="level 3":gameRun(m))
    level3_btn.place(x=850,y=20)
    level4_btn = tk.Button(frame3,text='Level 4',bg='#F29062',font=('Cambria',40),command=lambda m="level 4":gameRun(m))
    level4_btn.place(x=300,y=220)
    level5_btn = tk.Button(frame3,text='Level 5',bg='#F29062',font=('Cambria',40),command=lambda m="level 5":gameRun(m))
    level5_btn.place(x=700,y=220)
...
def mainwindow():
    ...
    global Game_tab
    global Dict_tab
    #defining the main window2
    window1=tk.Tk()
    window1.title('Insignia')
    window1.geometry('1250x700')
    window1.iconbitmap('D:/Reliance school/Computer project/extra/Danieledesantis-Audio-Video-Outline-Play.ico')
...
def login():
    ...
    if len(username)<1 or len(password)<1:
        tkinter.messagebox.showinfo("Error!","You have left either of the fields blank.\nPlease enter again!")
    else:
        if (username,) in rs1:
...
def sign():
    ...
    rs3=cursor.fetchall()
    c=rs3[0][0] #this helps to identify which user number are we catering to
    if (username,) in rs1:
             tkinter.messagebox.showerror("Error!",f"Sorry!The username {username} is not available\nPlease Enter Again!",parent=signup_window)
             name2_ent.delete('0','end')
             pswd2_ent.delete('0','end')
    elif len(username)<1 or len(username)>20:
             tkinter.messagebox.showerror("Error!","The username should be atleast 1 and atmost 20 characters long.\nPlease Enter Again!",parent=signup_window)
             name2_ent.delete('0','end')
             pswd2_ent.delete('0','end')
    elif len(password)<8 or len(password)>20:
             tkinter.messagebox.showerror("Error!","The password should be atleast 8 and atmost 20 characters long.\nPlease Enter Again!",parent=signup_window)
             name2_ent.delete('0','end')
             pswd2_ent.delete('0','end')
    else:
        cursor.execute(f"insert into user(user_id,username,pswd) values({c+1},'{username}','{password}')")
        sql_obj2.commit()
        cursor.execute(f"insert into level1(user_id,highscore) values({c+1},0)")
        sql_obj2.commit()
        cursor.execute(f"insert into level2(user_id,highscore) values({c+1},0)")
        sql_obj2.commit()
        cursor.execute(f"insert into level3(user_id,highscore) values({c+1},0)")
        sql_obj2.commit()
        cursor.execute(f"insert into level4(user_id,highscore) values({c+1},0)")
        sql_obj2.commit()
        cursor.execute(f"insert into level5(user_id,highscore) values({c+1},0)")
        sql_obj2.commit()
        sql_obj2.close()
        tkinter.messagebox.showinfo("Successful Signup!","Account successfully created!\nTaking you back to login page",parent=signup_window)
...
pswd1_ent=tk.Entry(frame3,width=30,font=('Cambria',20),show='*')
